...
def emit_cpp(model,out_dir:Path,xml_basenames):
    # Prefer a single umbrella header if present (generated by emit_autoreg.py).
    umbrella = 'idl_generated_includes.hpp'
    incs = ['#include "idl_json_bind.hpp"']
    # If the umbrella header exists in the out_dir, include it; otherwise fall back
    # to including each per-IDL header. The existence check happens at generation
    # time (file may not yet exist on first run), so prefer umbrella when possible.
    try:
