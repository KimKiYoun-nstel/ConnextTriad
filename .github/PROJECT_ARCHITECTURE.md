...
- **RtpDdsGateway**
  ...
  - 주요 구성요소:
    ...
    - `tools/*py` : 해당 py에서 생성하는 코드 `<build>/Idlkit/gen/idl*` 파일을 이용하여 DDS 타입과 JSON/문자열 변환
    - `type_registry`, `dds_type_registry`: 토픽/타입별 등록, 엔티티 생성, publish/take 헬퍼 등을 중앙에서 관리. 신규 타입/토픽 추가 시 레지스트리에 등록.
  - `Idlkit/idl/*.idl`에 정의된 IDL 타입 사용(예: `AlarmMsg.idl`, `StringMsg.idl`)
...
- **IDL → 생성 코드**: `rtiddsgen`이 Modern C++ 코드를 `${binary_dir}/Idlkit/gen/*`에 생성
- 생성된 헤더(`*.hpp`)는 실제 데이터 타입을 제공. 생성 소스(`*.cxx`, `*Plugin.*`, `*Support.*`)는 빌드 산출물로, 직접 수정하지 않음
- CMake는 RTI 라이브러리 디렉토리를 자동 감지(우선순위: `x64Win64VS2017/2019/2022`) 후 `nddscpp2`, `nddsc`, `nddscore`를 링크. `NDDSHOME` 환경변수 필요
...
1. `Idlkit/idl/.idl` 파일을 추가하고 VCS에 커밋
2. CMake/rtiddsgen으로 재설정/생성 → 헤더가 `<build>/Idlkit/gen/`에 생성됨
3. `dds_manager` 동작 및 UI RPC 바인딩에서 사용
4. 생성 소스는 직접 수정하지 않음
...
- **Don't**: `<build>/Idlkit/gen/` 파일은 헤더만 참조(읽기 전용), 스키마 변경은 `.idl`에서만
